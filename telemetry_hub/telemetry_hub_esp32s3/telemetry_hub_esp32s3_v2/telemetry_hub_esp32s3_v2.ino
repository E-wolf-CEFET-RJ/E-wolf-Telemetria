// ===== ESP32-S3 N16R8 — telemetry_hub_esp32s3_v2.ino =====
...
// escolha pinos LIVRES no S3
static const int LORA_RX_PIN = 18; // ESP32 RX  <- TX do LoRa
static const int LORA_TX_PIN = 17; // ESP32 TX  -> RX do LoRa
static const int LORA_BAUD   = 9600;
...
struct __attribute__((packed)) LoraTelemetryFrame {
  ...
  uint16_t vbatt_mv;   // g_volts * 1000
  int16_t  ibatt_ma;   // g_current_bat_a * 1000
...
void loraSendTelemetry() {
  LoraTelemetryFrame f;
  f.sync = 0xAA;
  f.seq  = loraSeq++;

  f.rpm       = (uint16_t)g_rpm;
  f.vbatt_mv  = (uint16_t)(g_volts * 1000.0f);
  f.ibatt_ma  = (int16_t)(g_current_bat_a * 1000.0f);

  f.flags = 0;
  if (g_override)          f.flags |= (1 << 0);
  if (g_ble_remote_mode)   f.flags |= (1 << 1);
  if (bleClientConnected)  f.flags |= (1 << 2);
  if (mqtt.connected())    f.flags |= (1 << 3);
  if (log_enabled)         f.flags |= (1 << 4);

  f.crc = lora_crc8((uint8_t*)&f, sizeof(f) - 1);

  SerialLoRa.write((uint8_t*)&f, sizeof(f));
}
