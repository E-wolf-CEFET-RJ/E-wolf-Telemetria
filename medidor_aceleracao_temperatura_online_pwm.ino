...
// Acelerador A0 + DHT D4 + PWM motor em D6 (IRLZ44N)
...
// ==== MOTOR / PWM ====
const uint8_t PWM_PIN = D6;      // GPIO12 -> Gate IRLZ44N via 100Ω
const int PWM_MAX = 1023;        // 10 bits no ESP8266
const int PWM_FREQ = 2000;       // ~2 kHz (bom para motor)
const int RAMP_STEP = 12;        // passo de rampa (contagens)
const uint32_t RAMP_MS = 10;     // taxa da rampa
int dutyTarget = 0;              // alvo (0..1023)
int dutyNow    = 0;              // atual (0..1023)
uint32_t lastRamp = 0;
...
// Faixa típica (ajustável pelos botões da página)
float V_MIN_REAL = 0.855f;
...
function draw(){
  ...
  function plot(arr, color, maxY){
    ...
    for(let i=0;i<n;i++){
      let v=arr[i]; if(v==null||Number.isNaN(v)) continue;
      if(maxY>0) v=v/maxY;
...
async function tick(){
  try{
    ...
    draw();
  }catch(e){ statusEl.textContent='status: ERRO ao buscar /data → '+e.message; }
  schedule();
}
schedule();

btnMin.onclick=async()=>{ try{ await fetch('/min_now'); statusEl.textContent='status: MIN atualizado'; }catch(_){ } };
btnMax.onclick=async()=>{ try{ await fetch('/max_now'); statusEl.textContent='status: MAX atualizado'; }catch(_){ } };
btnReset.onclick=async()=>{ try{ await fetch('/defaults'); statusEl.textContent='status: MIN/MAX restaurados'; }catch(_){ } };
ivalBtn.onclick=()=>{ const ms=parseInt(ivalIn.value||"1000",10); POLL_MS=Math.max(200,Math.min(10000,ms));
  statusEl.textContent='status: intervalo = '+POLL_MS+' ms'; schedule(); };
</script>
</body></html>
)";
...
// ==== MOTOR / PWM ====
// converte g_pct (0..100) para duty (0..1023)
...
void setup() {
  ...
  analogWrite(PWM_PIN, 0);  // motor parado
...
void loop() {
  ...
  // ==== MOTOR / PWM – rampa suave a cada RAMP_MS ====
  if (millis() - lastRamp >= RAMP_MS) {
    lastRamp = millis();
    if (dutyNow < dutyTarget) {
      dutyNow = min(dutyNow + RAMP_STEP, dutyTarget);
      analogWrite(PWM_PIN, dutyNow);
    } else if (dutyNow > dutyTarget) {
      dutyNow = max(dutyNow - RAMP_STEP, dutyTarget);
      analogWrite(PWM_PIN, dutyNow);
    }
  }
}
