...
| `speedometer.html` | HUD Web (Android + Chrome/Edge, via Web Bluetooth) | Velocímetro automotivo em **tela cheia**, projetado como **HUD de competição** para protótipos tipo Shell Eco-Marathon (cockpit baixo e estreito). Conecta **diretamente via Bluetooth Low Energy (Nordic UART Service)** ao ESP32, sem Wi-Fi, servidor ou app nativo. Exibe velocímetro gráfico com arco de 240°, ponteiro animado, **labels numéricos**, **shift-light por RPM**, modo dia/noite automático, indicadores de temperatura e corrente, além de modos alternáveis (velocidade, RPM, aceleração). Interface minimalista, responsiva e otimizada para leitura rápida em pista, com configurações persistentes via `localStorage`. |
